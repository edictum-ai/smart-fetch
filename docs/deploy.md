...
- **Always pass `-var smart_fetch_desired_count=1`** (default is 0 = paused) — the script does this.
...
until aws ecr list-images --repository-name personal-memory-prod-smart-fetch \
...
tofu apply -var smart_fetch_desired_count=1 -var smart_fetch_image_tag=$TAG \
           -var smart_fetch_browser_image_tag=6f3b58c -auto-approve
aws ecs update-service --cluster personal-memory-prod-smart-fetch \
  --service personal-memory-prod-smart-fetch --force-new-deployment \
